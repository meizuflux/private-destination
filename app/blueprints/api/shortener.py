...
from app.models.shortener import ShortenerAliasSchema, ShortenerCreateSchema
...
async def create_short_url_(request: web.Request) -> web.Response:
    ...
    try:
        await insert_short_url(
            request.app["db"], owner=request["user"]["id"], alias=alias, destination=json["destination"]
        )
