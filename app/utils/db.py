...
async def select_short_url_count(
    conn: ConnOrPool,
    *,
    owner: int
) -> int:
...
async def select_short_url_exists(
    conn: ConnOrPool,
    *,
    key: str
):
...
async def select_short_url_destination(
    conn: ConnOrPool,
    *,
    key: str
):
...
async def get_hash_and_id_by_email(
    conn: ConnOrPool,
    *,
    email: str
):
...
async def select_session_exists(
    conn: ConnOrPool,
    *,
    token: UUID
):
...
async def select_user_by_session(
    conn: ConnOrPool,
    *,
    token: UUID,
    scopes: Scopes
...
async def select_sessions(
    ...
    *,
    ...
    return await conn.fetch("SELECT token, created, browser, os FROM sessions WHERE user_id = $1 ORDER BY created DESC", user_id)

async def select_api_key_exists(
    conn: ConnOrPool,
    *,
    api_key: str
):
...
async def select_user_by_api_key(
    conn: ConnOrPool,
    *,
    api_key: str,
    scopes: Scopes
